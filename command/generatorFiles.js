...
class GenreatorFiles {
  ...
  async init(m) {
    ...
    console.log('build start, mode:', m);
    this.mode = m
    // 每次调用都是最新的
    ...
    console.log('build files start:');
    // 2. 遍历文件，批量产出转化后的文件
    await this.buildFiles()

    console.log('write readme start!');
    // 3. 将文件内的注释内容生成至readme.md，并进行组装
    this.writeReadme()

    console.log('record entry start!');
    // 4. 批量记录至入口文件
...
`
    ...
    // 写入ts入口文件，以此进行dts生成
    writeFile(`packages/${this.mode}/es/index.ts`, esContentData)
    // 生成dts
    await this.generateIndexDts()
  ...
  // 根据index.ts入口生成dts
  async generateIndexDts() {
    try{
      const dtsBuild = await rollup({
        input: `packages/${this.mode}/es/index.ts`,
        // file: '',
        plugins: [dts(), del({
          targets: [`packages/${this.mode}/es/index.ts`],
          hook:'buildEnd'
        })]
      })
      await dtsBuild.write({ dir: `packages/${this.mode}`, format: 'es' })
    }catch(e){
      console.error(e);
    }
  }
