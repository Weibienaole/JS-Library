import * as TDs from '../typings/devtools'
...
function allEquals(a: any, b: any): boolean {
  if (a === b) return true
...
const getColonTimeFromDate: TDs.GetColonTimeFromDate = (date) => {
  return date.toTimeString().slice(0, 8)
}

// 返回日期间的天数
// getDaysDiffBetweenDates4(new Date('2019-01-01'), new Date('2019-10-14'));
const getDaysDiffBetweenDates: TDs.GetDaysDiffBetweenDates = (
  dateInitial,
  dateFinal
) => {
...
// 时间格式转化年 -- 接受 ms 级的时间戳
const changeTimeYear: TDs.ChangeTimeYear = (time) => {
...
const formateSeconds: TDs.FormateSeconds = (surPlusTime) => {
  let secondTime = 0
...
// 时间戳转化为当前时间
const formatNowTime: TDs.FormatNowTime = (time) => {
...
// 返回 {x: , y: }
const getScrollPosition: TDs.GetScrollPosition = (el = window) => {
  return {
    x:
      (el as TDs.GetScrollPositionWin).pageXOffset !== undefined
        ? (el as TDs.GetScrollPositionWin).pageXOffset
        : (el as HTMLElement).scrollLeft,
    y:
      (el as TDs.GetScrollPositionWin).pageYOffset !== undefined
        ? (el as TDs.GetScrollPositionWin).pageYOffset
...
// 固定滚动条  接受一个y轴 Number
const preventScroll = (scrollNum: number): void => {
  // 存储当前滚动位置
...
*/
const formToObject: TDs.FormToObject = (form) => {
  return Array.from(new FormData(form) as any).reduce(
...
const lazyImage: TDs.LazyImage = (className = null) => {
  ...
  // 节流
  const throttle: TDs.LazyImageThrottle = (fn, delay = 100, mustRun = 30) => {
    let t_start = 0
...
const debounce: TDs.Debounce = (func, wait) => {
  let timer: NodeJS.Timeout
...
const throttle: TDs.Throttle = (func, wait, options = {}) => {
 ...
 * })
 */
function infinityScrolling(dom: HTMLElement, cb: Function) {
  // 监听滚动条要做的事情
  const monitorScroll = () => {
...
const getDomStyle = (el: HTMLElement, ruleName: String) => {
...
*/
   ...
   * @param {*} callback
   */
  const andoirFunction = (callback: Function) => {
   ...
   * @param {*} callback
   */
  const iosFuntion = (callback: Function) => {
    if (window.WebViewJavascriptBridge) {
  ...
  if (isAndroid) {
    ...
    window.setupWebViewJavascriptBridge(function (bridge: any) {
      ...
      bridge.init(function (msg: any, responseCallback: Function) {
...
const JSB_appMethod = (name: string, data = null) => {
  ...
  return new Promise(function (reslove, reject) {
    if (!window.setupWebViewJavascriptBridge)
      return reject('请先将 JSBriged.js 引入！')
    window.setupWebViewJavascriptBridge(function (bridge: any) {
