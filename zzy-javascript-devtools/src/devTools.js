...
*/
function generateTree(items, parentId = null, selectVal = 'parentId') {
...
function formateSeconds(surPlusTime) {
...
// 固定滚动条  接受一个y轴 Number
function preventScroll(scrollNum) {
  // 存储当前滚动位置
...
function recoverScroll(scrollNum) {
...
function lazyImage(className = null) {
 ...
 * })
 */
function infinityScrolling(dom, cb) {
  const that = this
...
function getDomStyle(el, ruleName) {
...
*/
function JSB_appMethod(name, data = null) {
  /**
   * name: 事件名
   * data: 参数 - 仅有调app事件持有
   */
