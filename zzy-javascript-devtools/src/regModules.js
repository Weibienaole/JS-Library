...
// 验证迅雷链接
function isThunderLink(val) {
  return /^thunderx?:\/\/[a-zA-Z\d]+=$/g.test(val);
}
// 验证window"文件夹"路径
function isWindowsFolderPath(val) {
  return /^[a-zA-Z]:\\(?:\w+\\?)*$/g.test(val);
}
// 验证window下"文件"路径
function isWindowsFilePath(val) {
